...
function Create(props) {
    ...
    axios({
      ...
      .then(function (response) {
        ...
        for (var i = 0; i < response.data.languages.length; i++) {
          $(
            ".contStyle label:nth-child(" +
              response.data.languages[i].lan_no +
              ")>input"
          ).prop("checked", true);
        }
        $("#addMbrBox").empty();
        for (var j = 0; j < response.data.userVOs.length; j++) {
          userArr.push(response.data.userVOs[j].mbr_no);
          if (response.data.userVOs[j].mbr_no == getCookie("mbr_no")) {
            $("#addMbrBox").append(
              "<div class='createProfile' data-mbrNo=" +
                response.data.userVOs[j].mbr_no +
                ">" +
                "<img class='avatar' src='https://img.icons8.com/color/36/000000/administrator-male.png' alt='...'/>" +
                "<div class='createList'>" +
                response.data.userVOs[j].mbr_nm +
                "(" +
                response.data.userVOs[j].mbr_email +
                ")" +
                "</div>" +
                "</div>"
            );
  ...
  const Save = () => {
    ...
    SaveForm.elements.prj_start_date.value = $(".Save_prj_start_date").val();
    SaveForm.elements.prj_end_date.value = $(".Save_prj_end_date").val();
    SaveForm.elements.mbr_nos.value = userArr.join();
  ...
  const [prj_lan_nm_text, set_prj_lan_nm_text] = useState(
    ProjectInfo.prj_lan_nm
  );
  const [prj_start_date_text, set_prj_start_date_text] = useState(
    ProjectInfo.prj_start_date
  );
  const [prj_end_date_text, set_prj_end_date_text] = useState(
    ProjectInfo.prj_end_date
  );
        ...
        for (var i = 0; i < response.data.length; i++) {
          $("#userSearchList").append(
            "<li class=userSearchListItem>" +
              "<div class=addUserClickBox data-mbr_no=" +
              response.data[i].mbr_no +
              " data-mbr_nm=" +
              response.data[i].mbr_nm +
              " data-mbr_email=" +
              response.data[i].mbr_email +
              "></div>" +
  ...
  $("#root").on("click", ".userSearchListItem", function (e) {
    ...
    if (!checkArr.includes(e.target.dataset.mbr_no)) {
      ...
      $("#addMbrBox").append(
        "<div class='createProfile'>" +
          "<img class='avatar' src='https://img.icons8.com/color/36/000000/administrator-male.png' alt='...'/>" +
          "<div class='createList' data-mbrNo=" +
          e.target.dataset.mbr_no +
          ">" +
          e.target.dataset.mbr_nm +
          "(" +
  ...
  $("#root").on("click", ".delMbrBtn", function (e) {
    userArr.splice(userArr.indexOf(e.target.parentElement.dataset.mbr_no), 1);
    ...
    <div>
      ...
      <div className="col-12 row mg-0 pd-15 ">
        <form
          id="SaveForm"
          method="post"
          action="http://localhost:8080/insertProjectInfo.staff"
        >
          ...
          <input type="hidden" name="mbr_nos" />
        </form>
        ...
        <div className="col-3 col-3-border proSidebar">
          <div className="createUser">
            <img
              ...
              alt="..."
            />
            {userInfo.mbr_nm}({userInfo.mbr_email})
            <div className="createList"></div>
          </div>
          ...
          <div className="makeProject pd-15">
            ...
            <h1 className="titleProject">
              ...
              <div className="nameProject">
                ...
                <div className="col-12 pd-0 pb-20">Project name</div>
                <input
                  type="text"
                  id="projectname"
                  className="width-100 pd-15 Save_prj_nm"
                ...
                <div className="nameProject">
                  ...
                  <div className="periodProject">
                    <input
                      type="date"
                      className="pd-10 calender-font Save_prj_start_date"
                      style={{ border: "2px solid lightgray" }}
                    />
                    ~
                    <input
                      type="date"
                      className="pd-10 calender-font Save_prj_end_date"
                      style={{ border: "2px solid lightgray" }}
                    />
                  </div>
                  <div className="nameProject">
                    <div className="col-12 pd-0 psb-20">Project progress</div>
                    <input
                      type="text"
                      className="progressProject Save_prj_prog"
                    />
                    <span>/ 100</span>
                  </div>
                  <div className="nameProject">
                    <div className="col-12 pd-0 pb-20">Language</div>
                    <div className="stackProject">{<Check />}</div>
                  </div>
                  <div className="nameProject pb-20">
                    <div className="col-12 pd-0 pb-20">Project purpose</div>
                    <input
                      type="text"
                      className="proposeProject pb-20 Save_prj_expl"
                    ></input>
                  </div>
                  <div>
                    <button
